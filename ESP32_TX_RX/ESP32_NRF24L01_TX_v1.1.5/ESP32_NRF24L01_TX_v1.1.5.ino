...
int rx1Addr = 80; // Address in EEPROM to store the receiver address
int rx2Addr = 90; // Address in EEPROM to store the receiver address
int rx3Addr = 100; // Address in EEPROM to store the receiver address
...
String firmwareVersionMaster = "v1.1.5";
...
receiver_Type = (receiver_value == 0)?"CAR":(receiver_value == 1)?"DRONE":"TRUCK";
  ...
  Serial.println("Receiver Type...: " + String(receiver_Type));
...
void receiver_settings(){
          ...
          display.setTextSize(1);
          display.setCursor(3, 1);//COL,ROW
          display.print("RX - [ "+ String(receiver_Type)+" ]");          
          display.setCursor(103,1);display.print(String(pagesCount) + "/" + String(totalPages));

          display.setTextColor(WHITE);
          display.setCursor(0,12); display.print(" CAR   : " + String(readRx1Address));   
          display.setCursor(0,21); display.print(" DRONE : " + String(readRx2Address)); 
          display.setCursor(0,31); display.print(" TRUCK : " + String(readRx3Address));




 binding_provision(bindingRx1Button, readRx1Address, rx1Addr, bindingRx1_address);
 binding_provision(bindingRx2Button, readRx2Address, rx2Addr, bindingRx2_address);
 binding_provision(bindingRx3Button, readRx3Address, rx3Addr, bindingRx3_address);
          ...
          case 1:
                 ...
                 switch(SubMenuSelect){
                     case 0:
                            display.setCursor(0,55);display.print("  PAIR  NO  UNPAIR");
                            break;
                     case 1:
...
void binding_TX_RX(String ReceiverType) {
...
struct BindData1 {
  String tx_type;     // Adjust the size based on your needs
  uint32_t tx_address;
}; 
...
//  bindData1.tx_type[sizeof(bindData1.tx_type) - 1] = '\0'; // Explicit null-termination
  bindData1.tx_type = ReceiverType;
  ...
  if (!timeout) {
    ...
    struct BindData2 {
      String rx_type;     // Adjust the size based on your needs
      uint32_t rx_address;
    };
...
//    if (strcmp(bindData2.rx_type, ReceiverType) == 0) {
   if (bindData2.rx_type == ReceiverType){
        
   
        if(bindingRx1Button == 1 && ReceiverType == "CAR") {
           writeUint32ToEEPROM(rx1Addr, bindData2.rx_address);
         }
        if(bindingRx1Button == 1 && ReceiverType != "CAR") {
           writeUint32ToEEPROM(rx1Addr, 0);
         }
                       
        if(bindingRx2Button == 1 && ReceiverType == "DRONE") {
           writeUint32ToEEPROM(rx2Addr, bindData2.rx_address);
         }
        if(bindingRx2Button == 1 && ReceiverType != "DRONE") {
           writeUint32ToEEPROM(rx2Addr, 0);
         }

        if(bindingRx3Button == 1 && ReceiverType == "TRUCK") {
           writeUint32ToEEPROM(rx3Addr, bindData2.rx_address);
         }
        if(bindingRx3Button == 1 && ReceiverType != "TRUCK") {
           writeUint32ToEEPROM(rx3Addr, 0);
         }         
...
//void binding_provision(int bindingRx1Button, uint32_t readRx1Address, int rx1Addr, int bindingRx1_address);

void binding_provision(int bindingButton, uint32_t readRxAddress, int rxAddr, int bindingRx_address){
  /**********************receiver 1*****************************************/
   if(bindingButton == 1 && readRxAddress == 0){
      int i = 0;
      while (i <= 50) { //2seconds
        Serial.println("Binding RX for 2 sec: " + String(i));
        display.clearDisplay(); 
        display.setTextColor(WHITE);
        display.setCursor(10,30); display.print("Binding RX...");display.print(i); 
        display.display();
        
//        binding_TX_RX(int receiverAddress, const char* ReceiverType);
          binding_TX_RX(receiver_Type);
        ...
        if (i == 50) {    
          ...
          display.setCursor(35,30); display.print("DONE");
          display.display(); 
          EEPROM.write(bindingRx_address, 0);
          EEPROM.commit();
          resetFunc();  //call reset
